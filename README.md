...
- [Autenticazione OpenSky](#autenticazione-opensky-opzionale)
- [Licenza](#licenza)
- [Crediti](#crediti)
- [Donazioni](#donazioni)
...
- 🎥 **Registrazione video** della sessione (solo schermo principale, anche su PC con più monitor collegati), con FFmpeg, metadati rimossi automaticamente
...
- 🔐 **Autenticazione OpenSky OAuth2** (opzionale): imposta le tue credenziali per un accesso più affidabile ai dati "mainstream" — funziona comunque anche senza, in modalità anonima
...
- **Registrazione video**: `FFmpeg` con cattura schermo (`gdigrab`), limitata al **solo schermo principale** (rilevato via API Windows) — su PC con più monitor, gli altri schermi non vengono inclusi nella registrazione; fermata in modo pulito (comando `q` su stdin) per non corrompere il file, poi ripulita dai metadati
...
## Autenticazione OpenSky (opzionale)

Dal 18 marzo 2026, OpenSky Network richiede un'autenticazione **OAuth2** (non più utente/password) per un accesso più affidabile ai dati "mainstream". SkyTruth **funziona comunque senza** (modalità anonima, con limiti più stretti), ma impostare le credenziali migliora l'affidabilità.

**Come ottenere le credenziali:**

1. Vai su [opensky-network.org](https://opensky-network.org/) e **registrati** (o accedi, se hai già un account)
2. Nella pagina del tuo account, cerca la sezione **"API Client"** e attivala/creala
3. **Scarica il file `credentials.json`** che ti viene proposto
4. Aprilo con **Blocco Note**: conterrà due valori, `clientId` e `clientSecret`

> ⚠️ Nella pagina OpenSky potresti vedere un avviso che dice di non usare il "Client ID" per scopi diversi dai sensori/feeder — non riguarda il nostro caso: per il semplice accesso ai dati (come fa SkyTruth), il Client ID mostrato nella sezione "API Client" è quello giusto da usare.

**Come inserirle in SkyTruth:**

Alla **prima volta** in cui apri la mappa "mainstream" in una sessione, compare un popup con due campi (**Client ID** e **Client Secret**) e due bottoni:
- **"Usa queste credenziali"** → le usa per tutta la sessione corrente, rinnovando automaticamente il token quando serve (circa ogni 30 minuti)
- **"Continua in modalità anonima"** → salta l'autenticazione, il programma funziona comunque

Le credenziali **non vengono mai salvate su disco**: vanno reinserite ad ogni riavvio del programma.

> 💡 Se possiedi già un dongle RTL-SDR funzionante (come richiesto per SkyTruth stesso), puoi valutare in futuro di "contribuire" i tuoi dati alla rete OpenSky Network (installando il loro programma feeder) — gli account che contribuiscono dati ottengono un limite giornaliero di richieste molto più alto.
