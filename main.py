...
def main():
    ...
    # Senza indicazioni esplicite, il driver DirectShow spesso sceglie
    # una modalita' di cattura piu' vecchia/stretta (es. 4:3, 640x480)
    # invece della modalita' widescreen piena che usa normalmente l'app
    # Fotocamera di Windows. Chiediamo esplicitamente il formato
    # panoramico 16:9 alla risoluzione piu' alta comune (1920x1080): se
    # la webcam non lo supporta, tornera' automaticamente alla
    # risoluzione piu' vicina disponibile, senza errori.
...
def avvia_registrazione_schermo():
    """
    Avvia la registrazione del SOLO schermo principale con FFmpeg
    (non l'intero desktop virtuale, che su un PC con piu' monitor
    includerebbe anche tutti gli altri schermi), salvando un file
    temporaneo nella cartella 'temp'. FFmpeg viene lanciato senza
    ...
    # SM_CXSCREEN (0) e SM_CYSCREEN (1): dimensioni del SOLO schermo
    # principale, non dell'intero desktop virtuale (che su un PC con
    # piu' monitor comprenderebbe anche tutti gli altri schermi
    # affiancati). Partendo sempre da offset (0,0), catturiamo solo
    # quell'area, indipendentemente da quanti altri monitor sono
    # collegati.
    larghezza_schermo_principale = ctypes.windll.user32.GetSystemMetrics(0)
    altezza_schermo_principale = ctypes.windll.user32.GetSystemMetrics(1)

    comando = [
        FFMPEG_EXE, "-y",
        "-f", "gdigrab", "-framerate", "15",
        "-offset_x", "0", "-offset_y", "0",
        "-video_size", f"{larghezza_schermo_principale}x{altezza_schermo_principale}",
        "-i", "desktop",
...
class MenuApp:
    ...
    def _chiedi_raggio(self):
        ...
        tk.Label(
            riga, text="Inserire il raggio di rilevamento in km (es. 250) e premere Invio:",
            font=FONT_MENU
        ).pack(side="left", padx=(0, 6))
